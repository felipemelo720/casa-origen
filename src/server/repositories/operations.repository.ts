...
export const paymentMethodRepository = {
  async findAllActive() {
    return prisma.paymentMethod.findMany({ where: { isActive: true }, orderBy: { sortOrder: 'asc' } });
  },
  async findAllForAdmin() {
...
export const businessHourRepository = {
  ...
  async upsertDay(dayOfWeek: number, data: { isClosed: boolean; opensAt: number; closesAt: number }) {
